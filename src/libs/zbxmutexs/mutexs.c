...
void	__zbx_rwlock_rdlock(const char *filename, int line, zbx_rwlock_t rwlock)
{
	...
	/* Readers are serialised with writers. A slim reader/writer lock would
	   let them run together, but it is not recursive, and the caches take a
	   read lock while already holding one. */
	EnterCriticalSection(&rwlock->lock);
}
...
void	zbx_rwlock_destroy(zbx_rwlock_t *rwlock)
{
	/* The table lives for the life of the process and is shared by every
	   worker, so the section itself is left alone; only this reference to it
	   is dropped. */
	*rwlock = ZBX_RWLOCK_NULL;
}
