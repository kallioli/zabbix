 ...
 * critical section in static storage covers the same ground.
 *
 * A slim reader/writer lock would let readers run together, and was used here
 * at first, but it is not recursive and the caches take a read lock in paths
 * that already hold one. A critical section is, and it is released the same way
 * it was taken, so nothing has to remember which mode is held.
 *
 * The Windows primitives are used directly rather than through the POSIX subset
 * in src/libs/zbxwin, because this translation unit is also linked into the
...
static INIT_ONCE		rwlocks_once = INIT_ONCE_STATIC_INIT;

static BOOL CALLBACK	rwlocks_init(PINIT_ONCE once, PVOID param, PVOID *context)
{
	int	i;
	...
	for (i = 0; i < ZBX_RWLOCK_COUNT; i++)
		InitializeCriticalSection(&rwlocks[i].lock);

	return TRUE;
}

static struct zbx_win_rwlock	*rwlock_get(zbx_rwlock_name_t name)
{
	InitOnceExecuteOnce(&rwlocks_once, rwlocks_init, NULL, NULL);
...
void	__zbx_rwlock_rdlock(const char *filename, int line, zbx_rwlock_t rwlock)
{
	...
	/* Readers are serialised with writers here. Letting them run together
	   needs a lock that is not recursive, and the caches take a read lock in
