...
/* acquire and per release. That overhead sat on top of the SQLite buffer's single-writer serialization and held the */
/* multi-sender ingest well below the Linux proxy, whose equivalent lock is a futex. A CRITICAL_SECTION acquires and  */
/* releases in user space when uncontended, which is the common path once WAL keeps the held section short.           */
/*                                                                                                                    */
/* Spin count is zero on purpose: a contended waiter goes straight to a kernel wait rather than burning a core        */
/* spinning. The section the SQLite mutex guards is a whole write transaction, far longer than a spin could bridge,   */
/* and an earlier build with a 4000-spin count measured slightly slower under heavy contention for exactly that.      */
/*                                                                                                                    */
...
#define ZBX_WIN_MUTEX_SPIN	0	/* no user-space spin; a contended waiter waits in the kernel at once */
