...
#ifdef _WINDOWS
/* The Windows transport uses loopback sockets and an in-process registry rather than a unix domain socket, so there */
/* is no sun_path to measure. The service path is only a naming key there; this keeps the same bound the rest of the */
/* code sizes its buffers against. */
